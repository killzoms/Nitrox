...
namespace NitroxPatcher.Patches.Dynamic
{
    ...
    {
        private static readonly MethodInfo targetMethod = typeof(GameSettings).GetMethod("SerializeInputSettings", BindingFlags.NonPublic | BindingFlags.Static);

        private const string SERIALIZER_FORMAT = "Input/Binding/{0}/{1}/{2}";
        ...
        public static void Postfix(GameSettings.ISerializer serializer)
        {
            ...
            foreach (GameInput.BindingSet bindingSet in Enum.GetValues(typeof(GameInput.BindingSet)))
            {
                foreach (KeyBinding keyBinding in keyBindingManager.KeyboardKeyBindings)
                {
                    ...
                    string name = string.Format(SERIALIZER_FORMAT, keyBinding.Device, keyBinding.Button, bindingSet);
